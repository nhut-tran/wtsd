...
if ( ! function_exists( 'wtsd_setup' ) ) :
	...
	function wtsd_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'wtsd' ),
			)
		);
 ...
 */
function wtsd_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'wtsd_content_width', 640 );
}
add_action( 'after_setup_theme', 'wtsd_content_width', 0 );
...
function wtsd_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'wtsd' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'wtsd' ),
